...
mission "translateMM01"
{
	...
	function void CreateRandomAttack(int nFromMarker, int nToMarker)
	{
		...
		if ( nTmp == 0 ) platTmp = CreateUnitsAtMarker(m_pEnemy, nFromMarker, "WHITEWOLF"     , 1+Rand(4));
		else if ( nTmp == 1 ) platTmp = CreateUnitsAtMarker(m_pEnemy, nFromMarker, "WHITEBEAR", 1+Rand(2));
		else if ( nTmp == 2 ) platTmp = CreateUnitsAtMarker(m_pEnemy, nFromMarker, "WHITEWOLF", 1+Rand(3));
		else if ( nTmp == 3 ) platTmp = CreateUnitsAtMarker(m_pEnemy, nFromMarker, "SKELETON1"     , 1        );
		else __ASSERT_FALSE();
    ...
    state Initialize
    {
		...
		m_auFootman[4] = CreateUnitAtMarker(m_pPlayer, 4, "NLOWCA");
		m_auFootman[5] = CreateUnitAtMarker(m_pPlayer, 5, "NWOJ");
		m_auFootman[6] = CreateUnitAtMarker(m_pPlayer, 6, "NWLOCZNIK");
		m_auFootman[7] = CreateUnitAtMarker(m_pPlayer, 7, "NRYCERZ");
	...
	event Timer5()
	{
		...
		platTmp = CreateUnitsAtMarker(m_pEnemy, 2, "SG_S_MS" , 1);
		CreateUnitsAtMarker(platTmp,  m_pEnemy, 2, "SG_W_GET", 1);
		
		CommandMoveAndDefendPlatoonToMarker(platTmp, 0);
		
		platTmp = CreateUnitsAtMarker(m_pEnemy, 3, "SG_S_MS" , 1);
		CreateUnitsAtMarker(platTmp,  m_pEnemy, 3, "RPG_HRABIA"  , 2);
		
		CommandMoveAndDefendPlatoonToMarker(platTmp, 1);
		
		platTmp = CreateUnitsAtMarker(m_pEnemy, 9, "SG_S_MS"   , 1);
		CreateUnitsAtMarker(platTmp,  m_pEnemy, 9, "SG_W_GET", 2);
	...
	event UnitDestroyed(unitex uUnit)
	{
		...
		for ( i=0; i<=7; ++i )
		{
			if ( uUnit == m_auFootman[i] )
			{
				if ( i == 7 && Rand(5) == 0 )
				{
					m_auFootman[i] = CreateUnitAtMarker(m_pPlayer, 8, "SG_W_GET");
				}
				else
				{
					nTmp = Rand(5+2+2+2);
					
					if ( nTmp <  5 ) m_auFootman[i] = CreateUnitAtMarker(m_pPlayer, 8, "NLOWCA"   );
					else if ( nTmp <  7 ) m_auFootman[i] = CreateUnitAtMarker(m_pPlayer, 8, "NWOJ");
					else if ( nTmp <  9 ) m_auFootman[i] = CreateUnitAtMarker(m_pPlayer, 8, "NWLOCZNIK"    );
					else if ( nTmp < 11 ) m_auFootman[i] = CreateUnitAtMarker(m_pPlayer, 8, "NRYCERZ"  );
					else __ASSERT_FALSE();
				}
