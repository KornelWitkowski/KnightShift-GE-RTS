...
mission "translateMM01"
{
	...
	function void CreateRandomAttack(int nFromMarker, int nToMarker)
	{
		...
		if ( nTmp == 0 ) platTmp = CreateUnitsAtMarker(m_pEnemy, nFromMarker, "RPG_FORESTS"     , 1+Rand(4));
		else if ( nTmp == 1 ) platTmp = CreateUnitsAtMarker(m_pEnemy, nFromMarker, "RPG_B4NDIT_WOODCUTTER", 1+Rand(2));
		else if ( nTmp == 2 ) platTmp = CreateUnitsAtMarker(m_pEnemy, nFromMarker, "RPG_B4NDIT", 1+Rand(3));
		else if ( nTmp == 3 ) platTmp = CreateUnitsAtMarker(m_pEnemy, nFromMarker, "RPG_B4NDIT_HUNTER"     , 1        );
		else __ASSERT_FALSE();
    ...
    state Initialize
    {
		...
		m_auFootman[4] = CreateUnitAtMarker(m_pPlayer, 4, "RPG_BANDIT_WOODCUTTER");
		m_auFootman[5] = CreateUnitAtMarker(m_pPlayer, 5, "RPG_BANDIT_HUNTER");
		m_auFootman[6] = CreateUnitAtMarker(m_pPlayer, 6, "RPG_BANDIT");
		m_auFootman[7] = CreateUnitAtMarker(m_pPlayer, 7, "RPG_BANDIT");
	...
	event Timer5()
	{
		...
		platTmp = CreateUnitsAtMarker(m_pEnemy, 2, "RPG_B4NDIT_WOODCUTTER" , 1);
		CreateUnitsAtMarker(platTmp,  m_pEnemy, 2, "RPG_B4NDIT_SPEARMAN", 1);
		
		CommandMoveAndDefendPlatoonToMarker(platTmp, 0);
		
		platTmp = CreateUnitsAtMarker(m_pEnemy, 3, "RPG_B4NDIT_WOODCUTTER" , 1);
		CreateUnitsAtMarker(platTmp,  m_pEnemy, 3, "RPG_B4NDITT"  , 2);
		
		CommandMoveAndDefendPlatoonToMarker(platTmp, 1);
		
		platTmp = CreateUnitsAtMarker(m_pEnemy, 9, "RPG_B4NDIT_WOODCUTTER"   , 1);
		CreateUnitsAtMarker(platTmp,  m_pEnemy, 9, "RPG_B4NDIT", 2);
	...
	event UnitDestroyed(unitex uUnit)
	{
		...
		for ( i=0; i<=7; ++i )
		{
			if ( uUnit == m_auFootman[i] )
			{
				if ( i == 7 && Rand(5) == 0 )
				{
					m_auFootman[i] = CreateUnitAtMarker(m_pPlayer, 8, "RPG_BANDIT");
				}
				else
				{
					nTmp = Rand(5+2+2+2);
					
					if ( nTmp <  5 ) m_auFootman[i] = CreateUnitAtMarker(m_pPlayer, 8, "RPG_BANDIT_WOODCUTTER"   );
					else if ( nTmp <  7 ) m_auFootman[i] = CreateUnitAtMarker(m_pPlayer, 8, "RPG_BANDIT_HUNTER");
					else if ( nTmp <  9 ) m_auFootman[i] = CreateUnitAtMarker(m_pPlayer, 8, "RPG_BANDIT_SPEARMAN"    );
					else if ( nTmp < 11 ) m_auFootman[i] = CreateUnitAtMarker(m_pPlayer, 8, "RPG_BANDIT"  );
					else __ASSERT_FALSE();
				}
